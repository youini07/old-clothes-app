...
function Home() {
  return (
    <div className="min-h-screen flex justify-center bg-[#F0EDE6]">
      {/* 모바일 화면 비율을 유지하는 컨테이너 (최대 너비 450px) */}
      <div className="w-full max-w-[450px] relative bg-[#F0EDE6]">
        {/* 배경 이미지 전체를 img 태그로 넣어 비율을 고정합니다. */}
        <img 
        ...
        {/* 버튼 1: 카카오 로그인 (이미지의 빈 공간 비율에 맞춰 위치 고정) */}
        <div className="absolute left-0 right-0 px-8" style={{ top: '69%' }}>
          <a
        ...
        {/* 버튼 2: 관리자 로그인 (하단 텍스트 아래 빈 공간 비율에 맞춰 위치 고정) */}
        <div className="absolute left-0 right-0 px-8" style={{ top: '88%' }}>
          <Link
            to="/login"
