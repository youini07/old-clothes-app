...
router.get('/notices/:partnerId', async (req, res) => {
  try {
    ...
    const [posts, total] = await Promise.all([
      ...
      prisma.boardPost.count({ where: { type: 'NOTICE', partnerId } }),
    ]);
...
// 공지사항 작성 (사장님만)
router.post('/notices', authenticate, requireRole(['PARTNER']), async (req: AuthRequest, res) => {
  try {
    ...
    // 사장님 정보 조회 (이름 가져오기)
    const partner = await prisma.user.findUnique({ where: { id: req.user!.userId } });
    if (!partner) return res.status(404).json({ error: '사용자를 찾을 수 없습니다.' });
...
// 공지사항 수정 (작성자만)
router.put('/notices/:id', authenticate, requireRole(['PARTNER']), async (req: AuthRequest, res) => {
  try {
...
// 공지사항 삭제 (작성자만)
router.delete('/notices/:id', authenticate, requireRole(['PARTNER']), async (req: AuthRequest, res) => {
  try {
    const id = req.params.id as string;
    const post = await prisma.boardPost.findUnique({ where: { id } });
    if (!post) return res.status(404).json({ error: '게시글을 찾을 수 없습니다.' });
...
// - 사장님: 자기에게 온 문의만 보임
// - 슈퍼관리자: 전체 문의 조회 가능
router.get('/inquiries', authenticate, async (req: AuthRequest, res) => {
  try {
    const { userId, role, partnerId: tokenPartnerId } = req.user!;
    const page = parseInt(req.query.page as string) || 1;
    const limit = parseInt(req.query.limit as string) || 20;
    const filterPartnerId = req.query.partnerId as string; // 슈퍼관리자가 특정 파트너 필터링용

    let whereClause: any = { type: 'INQUIRY' };
    ...
    if (role === 'CUSTOMER') {
      ...
      if (filterPartnerId) whereClause.partnerId = filterPartnerId;
    } else {
...
router.get('/inquiries/unread-count', authenticate, requireRole(['PARTNER']), async (req: AuthRequest, res) => {
  try {
...
router.get('/inquiries/:id', authenticate, async (req: AuthRequest, res) => {
  try {
    const id = req.params.id as string;
...
router.post('/inquiries/:id/comments', authenticate, async (req: AuthRequest, res) => {
  try {
    const id = req.params.id as string;
    const post = await prisma.boardPost.findUnique({ where: { id } });
    if (!post) return res.status(404).json({ error: '게시글을 찾을 수 없습니다.' });
...
router.get('/reviews/:partnerId', async (req, res) => {
  try {
    ...
    const [posts, total] = await Promise.all([
      ...
      prisma.boardPost.count({ where: { type: 'REVIEW', partnerId } }),
    ]);

    // 평균 별점 계산
    const avgRatings = await prisma.boardPost.aggregate({
      where: { type: 'REVIEW', partnerId },
      _avg: {
...
router.post('/reviews', async (req, res) => {
  try {
    ...
    // 필수 값 검증
    if (!requestId) return res.status(400).json({ error: '수거 신청 ID가 필요합니다.' });
    ...
    if (!request) return res.status(404).json({ error: '수거 신청을 찾을 수 없습니다.' });
    if (!request.partnerId) return res.status(400).json({ error: '담당 업체가 없는 수거건입니다.' });
    ...
    const maskName = (name: string) => {
      if (!name || name.length < 2) return name || '고객';
...
// 리뷰 삭제 (사장님 또는 슈퍼관리자만)
router.delete('/reviews/:id', authenticate, requireRole(['PARTNER', 'SUPER_ADMIN']), async (req: AuthRequest, res) => {
  try {
    const id = req.params.id as string;
    const post = await prisma.boardPost.findUnique({ where: { id } });
    if (!post) return res.status(404).json({ error: '게시글을 찾을 수 없습니다.' });
