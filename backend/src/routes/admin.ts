...
// 0-1. 슈퍼관리자 강제 계정 생성
router.post('/super/accounts', authenticate, requireRole(['SUPER_ADMIN']), async (req: any, res: any) => {
  try {
    ...
    const hashedPassword = password ? await bcrypt.hash(password, 10) : await bcrypt.hash(phone, 10);
