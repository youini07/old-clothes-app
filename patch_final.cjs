...
// 2. Fetch today on mount
if (!code.includes('fetchDailyStats(todayStr)')) {
  code = code.replace(
    "if (socketParams) {",
...
`;
  ...
  code = code.substring(0, statsViewStart) + newStatsView + '\n        ' + code.substring(statsViewEnd);
}
